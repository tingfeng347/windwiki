...
正文位于 `docs/`；图片放在 `docs/public/images/`。

主要分类：`agent/`、`llm/`、`computer-vision/`、`slam/`、`point-cloud/`、`research/`、`engineering/`、`tools/`。

`docs/_nav.json` 管理顶部导航；`docs/_meta.json` 组织全局知识树；各分类 `_meta.json` 管理局部顺序。不要在 `rspress.config.ts` 维护庞大的导航数组。

# Writing Rules

1. 默认使用中文，必要时保留英文技术术语；重要概念首次出现可写作「上下文工程（Context Engineering）」。
...
2. 判断所属分类，使用 kebab-case 文件名写 Markdown。仅在顺序需要调整时修改 `_meta.json`。
...
9. 新增依赖前确认 Rspress 默认能力是否足够；涉及配置或插件升级时核对当前官方文档。
